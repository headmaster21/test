...
RUN apk add --update ca-certificates wget && \
    ...
    unzip consul.zip && \mv consul /bin/ && \
    rm -rf consul.zip && \cd /tmp && \
...
CMD [ "agent", "-data-dir", "/data", "-server", "-bootstrap-expect", "1","-ui-dir", "/ui", "-client=0.0.0.0"]echo "$CONSUL_UI_SHA256 *ui.zip" | sha256sum -c - && \unzip ui.zip && \mkdir -p /ui && \mv * /ui && \rm -rf /tmp/* /var/cache/apk/*EXPOSE 8300 8301 8301/udp 8302 8302/udp 8400 8500 8600 8600/udpVOLUME [ "/data" ]ENTRYPOINT [ "/bin/consul" ]CMD [ "agent", "-data-dir", "/data", "-server", "-bootstrap-expect", "1","-ui-dir", "/ui", "-client=0.0.0.0"]
